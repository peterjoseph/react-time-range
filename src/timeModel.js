...
function generateTimeIncrement(minIncrementProp, minTime, maxTime) {
    //get the minimum starting hour and minute
    let minHour = minTime.split(':')[0]
    let minMinute = minTime.split(':')[1]
    //get the maximum starting hour and minute
    ...
    for (let i = 0; i < 24; i++) {
        for (let j = 0; j < minuteIncrement; j++) {
            //check if time is in range of min and max
            let active = (Number(("0" + i).slice(-2) + ("0" + j * minIncrementProp).slice(-2)) >= Number(("0" + minHour).slice(-2) + ("0" + minMinute).slice(-2))) && (Number(("0" + i).slice(-2) + ("0" + j * minIncrementProp).slice(-2)) <= Number(("0" + maxHour).slice(-2) + ("0" + maxMinute).slice(-2)));
            const time = {
...
export function generateTimeObjects(props) {
    ...
    startTimeIncrement = generateTimeIncrement(props.minuteIncrement, props.minStartMoment, props.maxStartMoment);
    endTimeIncrement = generateTimeIncrement(props.minuteIncrement, props.minEndMoment, props.maxEndMoment);
